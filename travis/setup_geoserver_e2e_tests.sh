...
docker run -d --name=test_postgis -p $POSTGIS_PORT:5432 -e POSTGRES_PASSWORD=$POSTGIS_PASS -e POSTGRES_USER=$POSTGIS_USER postgis/postgis:9.6-2.5
docker run -d --name=test_geoserver -p 8181:8181 -p 8081:8081 -p 8082:8082 -p 8083:8083 -p 8084:8084 -e ENABLED_NODES=1 -e REST_NODES=1 -e MAX_MEMORY=512 -e MIN_MEMORY=512 -e NUM_CORES=2 -e MAX_TIMEOUT=60 tethysplatform/geoserver
sleep 30
